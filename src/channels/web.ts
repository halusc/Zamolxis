...
/* The model name inside a "via" string, e.g. "Cerebras (gpt-oss-120b)" -> "gpt-oss-120b". */
function viaShort(v){var m=String(v||'').match(/\\(([^)]+)\\)/);return m?m[1]:String(v||'')}
...
function renderAgentThread(name){el('loginner').innerHTML='';
  ...
  fetch('/api/agentmsgs?since=0',{headers:hdrs()}).then(function(r){return r.ok?r.json():[]}).then(function(ms){
    ...
    var any=false;AGENTLOG.forEach(function(m){if(m.from===name||m.to===name){var lbl=(m.from===name)?('\\uD83E\\uDD16 '+m.from+' \\u2192 '+m.to):(m.from+' \\u2192 '+name);var mo=add('bot',lbl,m.text);agentVia(mo,m.via);any=true}});
...
function selKV(id,opts,c){var h='<select id="s_'+id+'">';opts.forEach(function(o){h+='<option value="'+esc(o[0])+'"'+(o[0]===c?' selected':'')+'>'+esc(o[1])+'</option>'});return h+'</select>'}
...
function pollAgentMsgs(){fetch('/api/agentmsgs?since='+agentSince,{headers:hdrs()}).then(function(r){return r.ok?r.json():[]}).then(function(ms){if(!ms||!ms.length)return;ms.forEach(function(m){if(m.ts>agentSince)agentSince=m.ts;
  ...
  if(isAgentCid(cid)&&(m.from===agentNameOf(cid)||m.to===agentNameOf(cid))){var mo=add('bot',lbl,m.text);agentVia(mo,m.via)}
})}).catch(function(){})}
